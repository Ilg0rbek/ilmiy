...
const Doctorant = () => {
  ...
  const getUserDoctorant = () => {
    setLoading(true);
    axiosConfig
      .post("/auth/users", { user: userSearch, kurs: kursSearch })
      .then((res) => {
        setData(res.data);
        setLoading(false);
      })
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, row) => (
        <div>
          <EyeOutlined
            onClick={() => navigate(`/admin/doctorantlist/detail/${row._id}`)}
          />
  ...
  const cerateUser = () => {
    if (count !== "" && user !== "") {
      setChekIn(true);
      axiosConfig
        .post(`/auth/generate-student`, { count, user })
        .then((res) => {
        ...
        <div style={{ display: "flex", width: "50%" }}>
          {chekin ? (
            <div style={{ padding: "10px" }}>
              <h6>Foydalanuvchilar tayyorlanmoqda..</h6>
            </div>
          ) : (
            <div
              style={{
            ...
            >
              <select
                ...
                id=""
              >
                <option disabled selected style={{ fontSize: "14px" }}>
                  Tanlang
                </option>
              ...
              <input
                ...
                type="text"
                placeholder="Foydalanuvchilarni yaratish uchun kerakli raqamni kiriting"
              />
            </div>
          )}
          <button
